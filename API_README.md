...
- **POST /api/auth/login:** Xác thực người dùng, trả về thông tin cơ bản + `token` JWT và đồng thời set cookie `client_token` (HTTP-only, hết hạn sau 1h).
- **POST /api/auth/refresh:** Nhận `client_refresh_token` (HTTP-only cookie), xoay vòng và cấp lại `client` + refresh token mới nếu hợp lệ.
